"""Generate this site's kanji content from a directory of kanji source data.

The source data is one YAML file per character plus the KanjiVG stroke SVGs; it
is not checked out beside this repository, which carries the generated output
instead. Nothing here needs the source to build. This script is kept for the day
the source resurfaces and the generated content has to be rebuilt from it.

`--source` is therefore required and has no default. It once defaulted to a
sibling directory, which silently became wrong when that directory moved, and a
default pointing at a path that does not exist is worse than no default at all.

    python scripts/sync_kanji_data.py --source <path to the source data>
    python scripts/sync_kanji_data.py --source <path> --check   # fail if stale

The source directory is whatever holds `data/kanji/<level>/*.yml`,
`data/levels.yml` and `data/stroke/<codepoint>.svg`.
...
# stroke-order SVG
#
# KanjiVG ships a DOCTYPE with an internal subset, private kvg:* metadata and
# hard-coded black strokes; none of that belongs inline in a page.
# ---------------------------------------------------------------------------
...
def load_kanji(source: Path) -> dict[str, list[dict]]:
    ...
    for level in LEVELS:
        ...
        for path in sorted((source / "data" / "kanji" / level).glob("*.yaml")):
            ...
            # YAML 1.1 resolves a bare `on:` key to the boolean True. The source
...
def kanji_document(k: dict, level: str, index: int, previous, following, svg: str | None) -> str:
    front = {
        "layout": "kanji",
        # The URL carries the character itself, so a shared link is readable and
...
def main() -> int:
    ...
    parser.add_argument(
        "--source",
        required=True,
        help="path to the kanji source data (the directory holding data/kanji/)",
    )
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    if not (source / "data" / "kanji").is_dir():
        ...
        print("       pass --source with the path to the kanji source data", file=sys.stderr)
        return 2
    ...
    header = (
        "# GENERATED by scripts/sync_kanji_data.py from the kanji source data.\n"
    ...
    index_text = json.dumps(
        {"generated_from": "kanji source data", "count": len(index_rows), "docs": index_rows},
        ensure_ascii=False,
        separators=(",", ":"),
    )
    ...
    if args.check:
        if stale:
            print(f"STALE: {len(stale)} file(s) differ from the source data")
            for name in stale[:20]:
                print(f"  {name}")
            if len(stale) > 20:
                print(f"  ... and {len(stale) - 20} more")
            print(f"\nRun: python scripts/sync_kanji_data.py --source {args.source}")
            return 1
