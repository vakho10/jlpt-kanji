...
(function () {
  ...
  function score(doc, raw) {
    ...
    for (var k = 0; k < doc.m.length; k++) {
      ...
      if (meaning === lower) { best = Math.max(best, 500); break; }
  ...
  function run() {
    ...
    load().then(function (all) {
      ...
      render(scored.slice(0, MAX_RESULTS).map(function (x) { return x.d; }), query);
  ...
  results.addEventListener('keydown', function (e) {
    ...
    if (e.key === 'ArrowDown') { e.preventDefault(); (hits[idx + 1] || hits[0]).focus(); }
